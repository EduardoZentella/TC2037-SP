...
#|(define tokens 
  '(("/* Esto es un comentario*/" "comentario_bloque") 
    ))|#
...
(define (es-condicion)
  (when (member token '("variable" "entero" "flotante" "flotante_negativo" "real"))
    (set! token (car (sigToken))))
...
(define (es-bloque-while)
  (cond
    [(and (equal? token "broche_cierre")
         (not (member token '("tipo_Variable" "variable" "comentario_linea"
                              "comentario_bloque" "for" "while" "if"
                              "function" "llamada_funcion"))))
     (displayln (string-append "\nbloque cierre:  " token))
     (when (not(eq? token ""))
        (set! token (car (sigToken)))
        (displayln (string-append "\nBroche despues de cierre: " token)))]
    [else
     (es-sentencia)
     (es-bloque-while)]))
